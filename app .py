...
app.config['MAIL_USERNAME'] = 'israeaouifi@gmail.com'  # Nom d'utilisateur pour l'authentification SMTP
app.config['MAIL_PASSWORD'] = 'cpgi zhmm anof epkv'  # Mot de passe pour l'authentification SMTP
app.config['MAIL_DEFAULT_SENDER'] = 'israeaouifi@gmail.com'  # Adresse email par défaut pour l'expéditeur
...
@app.route('/register', methods=['GET', 'POST'])  # Route pour l'inscription avec support des méthodes GET et POST
def register():  # Fonction pour gérer l'inscription
    if request.method == 'POST':  # Vérification si la requête est de type POST (soumission du formulaire)
        username = request.form['username']  # Récupération du nom d'utilisateur depuis le formulaire
        ...
        existing_user = User.query.filter_by(email=email).first()  # Vérification si l'email existe déjà dans la base
        if existing_user:  # Si l'utilisateur existe déjà
            flash("Email déjà utilisé", "danger")  # Affichage d'un message d'erreur
            return redirect(url_for('register'))  # Redirection vers la page d'inscription

        new_user = User(username=username, email=email, password=password)  # Création d'un nouvel utilisateur
        db.session.add(new_user)  # Ajout de l'utilisateur à la session de la base de données
        db.session.commit()  # Enregistrement des changements dans la base de données
        flash("Inscription réussie!", "success")  # Affichage d'un message de succès
        return redirect(url_for('login'))  # Redirection vers la page de connexion

    return render_template('register.html')  # Rendu du template register.html pour afficher le formulaire
